...
export const useDeliveries = (status?: string, fromDate?: string, storeId?: string) => {
  return useQuery(['deliveries', status, fromDate, storeId], () => deliveriesService.getAll(status, fromDate, undefined, !status, storeId), {
    staleTime: 2 * 60 * 1000, // 2 minutos
    retry: 3, // Retry 3 times on failure
    retryDelay: (attemptIndex) => Math.min(1000 * 2 ** attemptIndex, 30000), // Exponential backoff
    refetchOnWindowFocus: false, // Don't refetch on window focus to reduce load
