...
# Verificar cambios en frontend/, backend/ o shared/
if git diff --name-only $PREVIOUS_COMMIT HEAD | grep -qE "^(frontend/|backend/|shared/)"; then
  echo "✅ Cambios detectados en frontend, backend o shared - Ejecutando build"
  exit 0  # Exit code 0 = SUCCESS → Vercel continúa con el build
else
  echo "⏭️  No hay cambios en frontend, backend ni shared - Saltando build"
